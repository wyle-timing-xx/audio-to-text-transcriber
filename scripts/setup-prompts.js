...
// 创建默认的提示词文件（如果不存在）
const defaultEnPrompt = path.join(transcriptionPromptsDir, 'Prompt_en.md');
const defaultZhPrompt = path.join(transcriptionPromptsDir, 'Prompt_zh.md');
...
if (!fs.existsSync(defaultEnPrompt)) {
  ...
  console.log('Created default English prompt file');
}

if (!fs.existsSync(defaultZhPrompt)) {
  fs.writeFileSync(defaultZhPrompt, defaultZhContent, 'utf8');
  console.log('Created default Chinese prompt file');
}

console.log('Transcription prompts setup complete!');
